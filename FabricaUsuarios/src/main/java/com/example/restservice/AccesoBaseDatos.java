...
public class AccesoBaseDatos {
	...
	public static Boolean setUsuario(Connection conn, String nombre, String apellidos, String dni, String email,
			String password, Date fechaDeNacimiento, String direccion, String tipo) throws Exception {
		Boolean esAgregado = false;
		PreparedStatement insertStatement = conn.prepareStatement(
				"INSERT INTO [dbo].[usuario] (userDni, pass, nombre, email, apellidos, fechaNacimiento, direccion, tipo) VALUES (? , ? , ? , ? , ? , ? , ?)");
		try {
	...
	public static Usuario getUsuario(Connection conn, String user, String tipoUsuario) throws Exception {
		...
		String direccion = resultSet.getString("direccion");
		String tipo = resultSet.getString("tipo");
		// Se podria hacer un query con el user, tipo y contrasenia, asi un user puede
		// pertenecer a distintos roles.
	...
	public static Boolean modificarUsuario(Connection conn, Usuario usuarioAntiguo, Usuario usuarioNuevo)
			throws Exception {
		Boolean haSidoModificado = false;

		PreparedStatement updateStatement = conn.prepareStatement(
				"UPDATE [dbo].[usuario] SET userDni = ?, nombre = ?, pass = ?, email = ?, apellidos = ?, fechaNacimiento = ?,"
	...
	public static List<Cita> obtenerCitas(Connection conn) throws Exception {
		...
		PreparedStatement query = conn.prepareStatement(
				"SELECT * from [dbo].[usuario], [dbo].[citas] where userDni = dniMedico and tipo = 'medico';");
		ResultSet resultSet = null;
		try {
			resultSet = query.executeQuery();
		} catch (Exception e) {
	...
	public static List<Cita> citasReservadas(Connection conn, String dniPaciente) throws Exception {
		List<Cita> citas = new LinkedList<Cita>();
		PreparedStatement query = conn.prepareStatement(
				"SELECT * from [dbo].[usuario], [dbo].[citas] where userDni = dniMedico and tipo = 'medico' and dniPaciente = ?;");
		ResultSet resultSet = null;
		try {
	...
	public static List<Cita> citasDisponibles(Connection conn) throws Exception {
		List<Cita> citas = new LinkedList<Cita>();
		PreparedStatement query = conn.prepareStatement(
				"SELECT * from [dbo].[usuario], [dbo].[citas] where userDni = dniMedico and tipo = 'medico' and dniPaciente IS NULL;");
		ResultSet resultSet = null;
		try {
			resultSet = query.executeQuery();
		} catch (Exception e) {
	...
	public static Boolean anularCita(Connection conn, String user2, String cita2) throws Exception {
		Boolean haSidoModificado = false;
		PreparedStatement updateStatement = conn.prepareStatement("UPDATE [dbo].[citas] SET dniPaciente = NULL " +
				...
				"[dbo].[usuario] where tipo = 'paciente');");
		updateStatement.setString(1, user2);
		updateStatement.setString(2, cita2);
		updateStatement.setString(3, user2);
		updateStatement.setString(4, user2);
		try {
			updateStatement.executeUpdate();
			haSidoModificado = true;
		} catch (Exception e) {
		...
		if (haSidoModificado.equals(true)) {
			Usuario user = getUsuario(conn, user2, "paciente");
			System.out.println(user);
			Cita cita = obtenerCita(conn, cita2);
			email e = new email();
	...
	public static Boolean agregarCita(Connection conn, Date fechaInicio, Date fechaFin, String consulta,
	...
	public static Boolean eliminarCita(Connection conn, String idCita) throws Exception {
		...
		try {
			...
			throw new Exception("Error al intentar eliminar el usuario", e);
		}
		return esEliminado;
	}
	...
	public static Cita obtenerCita(Connection conn, String cita_id) throws Exception {
		Cita cita = null;
		PreparedStatement query = conn.prepareStatement(
				"SELECT * from [dbo].[usuario], [dbo].[citas] where userDni = dniMedico and tipo = 'medico' and id = ?;");
